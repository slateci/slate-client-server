...
TEST(UnauthenticatedReplaceUserToken){
	...
	auto resp=httpGet(tc.getAPIServerURL()+"/v1alpha1/users/User_1234/replace_token");
	...
	resp=httpGet(tc.getAPIServerURL()+"/v1alpha1/users/User_1234/replace_token?token=00112233-4455-6677-8899-aabbccddeeff");
	ENSURE_EQUAL(resp.status,403,
...
TEST(ReplaceNonexistentUserToken){
	...
	auto resp=httpGet(tc.getAPIServerURL()+"/v1alpha1/users/blah/replace_token?token="+adminKey);
...
TEST(AdminReplaceUserToken){
	...
	{ //check that the user has access
		auto listResp=httpGet(tc.getAPIServerURL()+"/v1alpha1/users?token="+originalToken);
		ENSURE_EQUAL(listResp.status,200,"User should be able to list users");
	}
	std::string newToken;
	{ //replace the user's token
		auto resp=httpGet(tc.getAPIServerURL()+"/v1alpha1/users/"+uid+"/replace_token?token="+adminKey);
	...
	{ //check that the new token does give access
		auto listResp=httpGet(tc.getAPIServerURL()+"/v1alpha1/users?token="+newToken);
