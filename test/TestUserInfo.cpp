...
TEST(UnauthenticatedGetUserInfo){
	...
	auto resp=httpGet(tc.getAPIServerURL()+"/v1alpha1/users/User_1234");
	...
	resp=httpGet(tc.getAPIServerURL()+"/v1alpha1/users/User_1234?token=00112233-4455-6677-8899-aabbccddeeff");
	ENSURE_EQUAL(resp.status,403,
...
TEST(NonexistentGetUserInfo){
	...
	auto resp=httpGet(tc.getAPIServerURL()+"/v1alpha1/users/blah?token="+adminKey);
...
TEST(GetUserInfo){
	...
	auto resp=httpGet(tc.getAPIServerURL()+"/v1alpha1/users/"+adminUID+"?token="+adminKey);
...
TEST(GetUserNewInfo){
	...
	//add a new user
	{
		...
		auto createResp=httpPost(tc.getAPIServerURL()+"/v1alpha1/users?token="+adminKey,to_string(request1));
	...
	auto resp=httpGet(tc.getAPIServerURL()+"/v1alpha1/users/"+uid+"?token="+adminKey);
	ENSURE_EQUAL(resp.status,200,
...
TEST(GetInfoAuthorization){
	...
	std::string adminKey=getPortalToken();
	std::string baseUserURL=tc.getAPIServerURL()+"/v1alpha1/users?token=";
	...
	auto resp=httpGet(tc.getAPIServerURL()+"/v1alpha1/users/"+uid1+"?token="+tok1);
	ENSURE_EQUAL(resp.status,200,"First user should be able to get own info");
	
	resp=httpGet(tc.getAPIServerURL()+"/v1alpha1/users/"+uid2+"?token="+tok2);
	ENSURE_EQUAL(resp.status,200,"Second user should be able to get own info");
	
	resp=httpGet(tc.getAPIServerURL()+"/v1alpha1/users/"+uid2+"?token="+tok1);
	ENSURE_EQUAL(resp.status,403,"First user should not be able to get second user's info");
	
	resp=httpGet(tc.getAPIServerURL()+"/v1alpha1/users/"+uid1+"?token="+tok2);
