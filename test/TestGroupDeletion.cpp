...
TEST(UnauthenticatedDeleteGroup) {
	...
	createResp = httpDelete(tc.getAPIServerURL() + "/" + currentAPIVersion +
	                        "/groups/Group_1234567890?token=00112233-4455-6677-8899-aabbccddeeff");
	ENSURE_EQUAL(createResp.status, 403,
...
TEST(NonmemberDeleteGroup) {
	...
	{ //create a user
		...
		request.AddMember("metadata", metadata, alloc);
		auto createResp = httpPost(tc.getAPIServerURL() + "/" + currentAPIVersion + "/users?token=" + adminKey,
		                           to_string(request));
...
TEST(DeletingGroupHasCascadingDeletion) {
	...
	{ // install an instance
		...
		auto instResp = httpPost(
				tc.getAPIServerURL() + "/" + currentAPIVersion + "/apps/test-app?test&token=" + adminKey,
				to_string(request));
	...
	stopReaper();
	ENSURE_EQUAL(names.output.find("slate-group-testgroup1"), std::string::npos,
	             "VO deletion should delete associated namespaces");
}
