...
TEST(AllocateClusterCredential) {
	...
	{
		auto clusterResp = httpGet(
				tc.getAPIServerURL() + "/" + currentAPIVersion + "/clusters/" + clusterID + "?token=" + adminKey);
	...
	S3Credential c1("foo", "bar");
	{
		...
		auto addResp = httpPost(
				tc.getAPIServerURL() + "/" + currentAPIVersion + "/monitoring_credentials?token=" + adminKey,
...
TEST(AllocateClusterCredentialUnavailable) {
	...
	//On to the main event!
	{
		auto credResp = httpGet(tc.getAPIServerURL() + "/" + currentAPIVersion + "/clusters/"
		                        + clusterID + "/monitoring_credential?token=" + adminKey);
		ENSURE_EQUAL(credResp.status, 500,
		             "Monitoring credential fetch request should fail when no credentials are available");
	}
}
...
TEST(NoDoubleAllocation) {
	...
	{
		auto clusterResp = httpGet(
				tc.getAPIServerURL() + "/" + currentAPIVersion + "/clusters/" + clusterID1 + "?token=" + adminKey);
		ENSURE_EQUAL(clusterResp.status, 200, "Cluster info request should succeed");
		rapidjson::Document clusterData;
		clusterData.Parse(clusterResp.body);
		ENSURE_EQUAL(clusterData["metadata"]["hasMonitoring"].GetBool(), true,
...
TEST(AllocateClusterCredentialRevoked) {
	...
	{ //revoke the credential
		auto revResp = httpPut(
				tc.getAPIServerURL() + "/" + currentAPIVersion + "/monitoring_credentials/" + c1.accessKey +
				"/revoke?token=" + adminKey, "");
