...
bool operator==(const directory_iterator& d1, const directory_iterator& d2){
	if (d1.dirp == NULL && d2.dirp == NULL) { return (true); }
	if (!!d1.curItem.item != !!d2.curItem.item) { return (false); }
	if (d1.curItem.item == NULL && d2.curItem.item == NULL) { return (true); }
