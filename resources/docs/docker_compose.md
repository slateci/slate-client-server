...
## Build the SLATE Client

If there are build artifacts in `${PWD}/build` not generated by `docker` remove them to prevent errors:

```shell
[your@localmachine]$ rm -rf ${PWD}/build/.*
```

* The `cmake` process builds against Alpine which can cause issues for artifacts already generated by other host operating systems.

Next exec into the Alpine container and build the client:
...
On the host `build/slate` will appear. Copy that to a new location for testing.
